...
public class Leapgesture2 : MonoBehaviour
{
        ...
        foreach (var itemHands in mFrame.Hands)
        {
            ...
            else if (itemHands.IsRight)
            {
                ...
                if (isCloseFullHand(itemHands))
                {
                    ...
                    Vector3 palmpos = new Vector3(itemHands.PalmPosition.x, itemHands.PalmPosition.y, itemHands.PalmPosition.z);
                    Vector2 screenpos = camera.WorldToScreenPoint(palmpos);
                    image.gameObject.transform.position = screenpos + offset;
